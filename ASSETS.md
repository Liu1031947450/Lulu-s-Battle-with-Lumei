...
开场另外使用四段 macOS **Tingting** 中文女声合成资源：`voice-3.wav`（三）、`voice-2.wav`（二）、`voice-1.wav`（一）、`voice-start.wav`（开始）。它们不是拳皇原声或真人录音；生成参数为 175 wpm，保留短促自然停顿，裁剪首尾静音、统一有效音量并作首尾淡入淡出，长度分别约 0.397 / 0.293 / 0.258 / 0.573 秒。

`npm run voices` 使用本机 `say` 重新生成这些文件，无在线服务或新增依赖。普通构建只校验并内嵌现成 WAV 的 PCM 数据；运行时不调用系统朗读、不请求外部资源。语音替代原倒数和开打提示音，缺失或损坏时回退原提示音，其他音乐音效保持不变。
